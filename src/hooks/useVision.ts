...
export function useVision(activeModule: ModuleType) {
  ...
  // Estado reactivo del elemento de video — garantiza que el Workspace siempre recibe el elemento
  // correcto aunque la ref se asigne después del primer render (fix para arranque directo sin menú).
  ...
  const getOrCreateVideoElement = (): HTMLVideoElement => {
    if (!videoRef.current) {
      ...
      // Disparar re-render para que el Workspace reciba el elemento de video real
  ...
  const startSensors = async () => {
    ...
    try {
      // Esperar a que los scripts de MediaPipe terminen de cargar en window.
      // Necesario cuando la app arranca directamente en un módulo (sin pasar por el menú).
      let waited = 0;
      while (
        (!(window as any).FaceMesh || !(window as any).Hands || !(window as any).Camera) &&
        waited < 10000
      ) {
      ...
      if (!faceMeshRef.current) {
        ...
        const faceMesh = new FaceMeshClass({
          locateFile: (file: string) => {
            // Extraer solo el nombre del archivo (MediaPipe a veces pasa rutas internas completas)
            const filename = file.split('/').pop() || file;
            return `/models/face_mesh/${filename}`;
          },
        });
        ...
        faceMesh.onResults((results: any) => {
          if (results && results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
            setFaceLandmarks(results.multiFaceLandmarks); // Pasamos TODAS las caras detectadas
