...
export function Workspace({
  ...
  const pointerRef = useRef({ x: -100, y: -100 }); // Suavizado
  ...
  const handleSaveCurrentEmotion = (forcedEmotion?: string) => {
    ...
    const faceData = faceLandmarksRef.current;
    const isMultiFace = Array.isArray(faceData[0]);
    const landmarks = isMultiFace ? faceData[0] : faceData;
    const keyPoints = {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (singleHand && singleHand.length > 8) {
        ...
        const targetX = (1 - indexFingerTip.x) * width;
        const targetY = indexFingerTip.y * height;
        
        // Low-pass filter (Lerp) para estabilizar el cursor y evitar saltos (0.4 es la velocidad de transición)
      ...
      if (activeModule === "FILTERS") {
        ...
        const faceDataList = faceLandmarksRef.current;
        if (faceDataList && faceDataList.length > 0) {
              ...
              if (sp.colorType) {
                ...
                if (sp.colorType === "electric") {
                  ...
                  if (sp.size > 2) {
                    ...
                    grad.addColorStop(0.3, `rgba(${sp.r}, ${sp.g}, ${sp.b}, ${sp.alpha * 0.8})`);
          ...
          // --- DIBUJO UNIVERSAL DE MANOS (LADO INGENIERÍA) ---
          if (fIndex === 0 && rawHands) {
            const handsList = Array.isArray(rawHands) ? rawHands : [rawHands];
            
            // Determinar paleta de colores de la mano según el mesh actual
            let boneColor = "rgba(255, 255, 255, 0.82)";
            let shadowColor = "#00d2ff"; // Cyan por defecto
            let nodeColor = "#00d2ff";
            
            if (meshType === "biolum") {
              shadowColor = "#10b981"; // Emerald green
              nodeColor = "#34d399";
              boneColor = "rgba(16, 185, 129, 0.6)";
            } else if (meshType === "fuego") {
              shadowColor = "#ef4444"; // Red
              nodeColor = "#f97316";   // Orange
              boneColor = "rgba(253, 224, 71, 0.6)"; // Yellowish
            } else if (meshType === "electrico") {
              shadowColor = "#eab308"; // Yellow
              nodeColor = "#fde047";
              boneColor = "rgba(255, 255, 255, 0.9)";
            } else if (meshType === "plexus") {
              const currentTheme = CYBER_THEMES[activeCyberThemeRef.current] || CYBER_THEMES[1];
              shadowColor = `rgb(${currentTheme.halo1})`;
              nodeColor = `rgb(${currentTheme.mask})`;
              boneColor = `rgba(${currentTheme.halo2}, 0.8)`;
            } else if (meshType === "cyber") {
              shadowColor = "#06b6d4"; // Cyan
              nodeColor = "#22d3ee";
              boneColor = "rgba(6, 182, 212, 0.5)";
            }
            ...
            handsList.forEach((hand) => {
              if (hand && hand.length > 0) {
                ...
                // Nodos exteriores de las articulaciones
                ctx.save();
      ...
      // MÓDULO 2: LECTOR DE EMOCIONES BIOMÉTRICO
        ...
        if (faceData && faceData.length > 0) {
          // Para el lector biométrico, analizamos solo a la persona principal (índice 0)
          const isMultiFace = Array.isArray(faceData[0]);
          const face = isMultiFace ? faceData[0] : faceData;
